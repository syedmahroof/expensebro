...
async function markAllRead() {
    await fetch('/notifications/read', {
        method: 'POST',
        headers: { 'X-Requested-With': 'XMLHttpRequest', 'Content-Type': 'application/json', 'X-XSRF-TOKEN': getCsrf() },
...
<template>
    <div class="relative" data-notification-bell>
        ...
        <Transition name="dropdown">
            <div
                v-if="open"
                class="absolute right-0 top-10 z-50 w-80 overflow-hidden rounded-xl border border-sidebar-border bg-card shadow-xl"
            >
                <div class="flex items-center justify-between border-b border-sidebar-border/50 px-4 py-3">
                    <p class="text-sm font-semibold">Notifications</p>
                    <button v-if="unreadCount > 0" @click="markAllRead" class="text-xs text-primary hover:text-primary/80">Mark all read</button>
                </div>
                ...
                <div v-else-if="notifications.length === 0" class="py-10 text-center text-sm text-muted-foreground">
                    No notifications
                </div>

                <div v-else class="max-h-72 divide-y divide-sidebar-border/30 overflow-y-auto">
                    <div
                    ...
                    >
                        <div class="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg" :class="typeBg(n.type)">
                            <component :is="typeIcon(n.type)" class="h-3.5 w-3.5" :class="typeColor(n.type)" />
                        </div>
                        <div class="min-w-0 flex-1">
                            <div class="flex items-start justify-between gap-2">
                                <p class="text-xs font-semibold" :class="n.read ? 'text-foreground/80' : 'text-foreground'">{{ n.title }}</p>
                                <span v-if="!n.read" class="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-primary" />
                            </div>
                            <p class="mt-0.5 text-xs text-muted-foreground leading-relaxed">{{ n.body }}</p>
                            <p class="mt-1 text-[10px] text-muted-foreground/60">{{ timeAgo(n.created_at) }}</p>
                        </div>
                    </div>
                </div>
                <div class="border-t border-sidebar-border/50 px-4 py-2.5">
                    <button
                        @click="router.visit(notificationsIndex().url); open = false"
