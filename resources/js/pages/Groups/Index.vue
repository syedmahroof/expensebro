...
async function deleteGroup(id: number) {
    const ok = await confirm(
        'This group and all its expenses will be permanently deleted.',
        {
            title: 'Delete group?',
            confirmText: 'Delete',
        },
...
<template>
    ...
    <div class="flex h-full flex-1 flex-col gap-5 p-4 md:p-6">
        ...
        <div class="grid gap-3 sm:grid-cols-2">
            ...
            <div class="rounded-xl border border-sidebar-border/50 bg-card p-4">
                ...
                <p class="mt-1.5 text-xl font-bold">
                    {{ groups.reduce((s, g) => s + g.members_count, 0) }}
                </p>
                <p class="mt-0.5 text-xs text-muted-foreground">
                    across all groups
                </p>
        ...
        >
            ...
            <button
                @click="openModal"
                class="mt-3 text-sm text-orange-400 hover:text-orange-300"
            >
        ...
        <div v-else class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            ...
            >
                <Link
                    :href="{ url: `/groups/${group.id}`, method: 'get' }"
                ...
                <div class="flex items-start justify-between gap-2">
                    <div class="flex items-center gap-2.5">
                        <div
                            class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-orange-500/15"
                        >
                    ...
                    <button
                        ...
                        class="relative z-10 shrink-0 text-muted-foreground opacity-0 transition-opacity group-hover:opacity-100 hover:text-red-400"
                    >
                ...
                <div class="mt-3 flex items-center justify-between gap-2">
                    ...
                    <p class="text-sm font-bold text-orange-400 tabular-nums">
                        {{ fmt(group.total_expenses, group.currency) }}
                    </p>
                </div>
            </div>
        </div>
