...
import {
    ArrowRight, CheckCircle2, Circle, Plus, Receipt, Trash2, Users, X,
...
import { destroy as destroyExpense, store as storeExpense } from '@/routes/groups/expenses';
...
const totalExpenses = computed(() =>
    props.expenses.reduce((sum, e) => sum + Number(e.converted_amount ?? e.amount), 0)
);
...
const form = useForm({
    ...
    notes: '',
    splits: props.members.map(m => ({ group_member_id: m.id, amount: '' as number | '' })),
});
...
function openExpenseModal() {
    ...
    form.splits = props.members.map(m => ({ group_member_id: m.id, amount: '' as number | '' }));
    showExpenseModal.value = true;
}

function splitEvenly() {
    if (!form.amount || props.members.length === 0) { return; }
    const share = Math.round((Number(form.amount) / props.members.length) * 100) / 100;
...
async function deleteExpense(expenseId: number) {
    const ok = await confirm('This expense and all its splits will be permanently removed.', {
        title: 'Delete expense?',
        confirmText: 'Delete',
    });
    if (!ok) { return; }
    router.delete(destroyExpense({ group: props.group.id, expense: expenseId }).url, { preserveScroll: true });
}

function toggleSplit(splitId: number) {
    router.patch(settleSplit({ group: props.group.id, split: splitId }).url, {}, { preserveScroll: true });
}
...
const settledSplitsCount = computed(() =>
    props.expenses.flatMap(e => e.splits).filter(s => s.settled_at).length
);
...
<template>
    ...
    <div class="flex h-full flex-1 flex-col gap-5 p-4 md:p-6">
        ...
        <div class="flex items-start justify-between gap-3">
            <div>
                <h1 class="text-xl font-bold tracking-tight">{{ group.name }}</h1>
                <p v-if="group.description" class="text-sm text-muted-foreground">{{ group.description }}</p>
                <p v-else class="text-sm text-muted-foreground">{{ members.length }} members · {{ group.currency }}</p>
            </div>
            <button
        ...
        <div class="grid gap-3 sm:grid-cols-3">
            <div class="rounded-xl border border-orange-500/20 bg-orange-500/5 p-4">
                ...
                <p class="mt-1.5 text-xl font-bold text-orange-400">{{ fmt(totalExpenses) }}</p>
                <p class="mt-0.5 text-xs text-muted-foreground">{{ expenses.length }} expense{{ expenses.length !== 1 ? 's' : '' }}</p>
            </div>
            <div class="rounded-xl border border-sidebar-border/50 bg-card p-4">
                <p class="text-xs text-muted-foreground">Per Member Share</p>
                <p class="mt-1.5 text-xl font-bold">{{ fmt(perMemberShare) }}</p>
                <p class="mt-0.5 text-xs text-muted-foreground">even split · {{ members.length }} people</p>
            </div>
            <div class="rounded-xl border border-sidebar-border/50 bg-card p-4">
                <p class="text-xs text-muted-foreground">Settlement Progress</p>
                <p class="mt-1.5 text-xl font-bold">{{ settledSplitsCount }}/{{ totalSplitsCount }}</p>
                <p class="mt-0.5 text-xs text-muted-foreground">splits settled</p>
            </div>
        </div>
        ...
        <div class="grid gap-5 lg:grid-cols-3">
            ...
            <div class="lg:col-span-2">
                <div class="mb-3 flex items-center gap-2">
                    <div class="flex h-6 w-6 items-center justify-center rounded-full bg-orange-500/15">
                        <Receipt class="h-3.5 w-3.5 text-orange-400" />
                    </div>
                    <h2 class="text-sm font-semibold">Expenses</h2>
                </div>

                <div v-if="expenses.length === 0" class="flex flex-col items-center justify-center rounded-xl border border-dashed border-sidebar-border/50 py-12 text-center">
                    <Receipt class="mb-2 h-8 w-8 text-muted-foreground/30" />
                    <p class="text-sm text-muted-foreground">No expenses yet</p>
                    <button @click="openExpenseModal" class="mt-2 text-xs text-orange-400 hover:text-orange-300">+ Add first expense</button>
                </div>
                    ...
                    >
                        <div class="flex items-start gap-3">
                            <div class="min-w-0 flex-1">
                                <div class="flex items-baseline justify-between gap-2">
                                    ...
                                    <div class="shrink-0 text-right">
                                        <p class="font-bold tabular-nums text-orange-400">{{ fmt(expense.converted_amount ?? expense.amount) }}</p>
                                        <p v-if="expense.currency !== group.currency" class="text-[10px] text-muted-foreground">{{ fmt(expense.amount, expense.currency) }}</p>
                                    </div>
                                </div>
                                <div class="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                                    <span>Paid by <strong class="text-foreground">{{ expense.paid_by_name }}</strong></span>
                                ...
                                <div class="mt-3 space-y-1">
                                    ...
                                    >
                                        <button
                                            ...
                                            :title="split.settled_at ? 'Mark as unsettled' : 'Mark as settled'"
                                        >
                                            <CheckCircle2 v-if="split.settled_at" class="h-4 w-4 text-emerald-400" />
                                            <Circle v-else class="h-4 w-4 text-muted-foreground/40 hover:text-emerald-400" />
                                        </button>
                                        <span class="min-w-0 flex-1 truncate text-xs" :class="split.settled_at ? 'line-through text-muted-foreground/50' : 'text-muted-foreground'">
                                            {{ split.member_name }}
                                        </span>
                                        <span class="text-xs tabular-nums font-medium" :class="split.settled_at ? 'line-through text-muted-foreground/50' : ''">
                                            {{ fmt(split.amount) }}
                                        </span>
                                    </div>
                                </div>
                            ...
                            <button
                                ...
                                class="shrink-0 text-muted-foreground opacity-0 transition-opacity hover:text-red-400 group-hover:opacity-100 mt-0.5"
                            >
            ...
            <div class="space-y-5">
                ...
                <div>
                    <div class="mb-3 flex items-center gap-2">
                        ...
                        <span class="rounded-full bg-sidebar-accent/60 px-2 py-0.5 text-[10px] font-medium">{{ members.length }}</span>
                    </div>
                    <div class="space-y-1.5">
                        ...
                        >
                            <div class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-sidebar-accent text-[10px] font-bold uppercase">
                                {{ member.name.slice(0, 2) }}
                            </div>
                            <div class="min-w-0 flex-1">
                                <p class="truncate text-xs font-medium">{{ member.name }}</p>
                                <p v-if="member.phone" class="text-[10px] text-muted-foreground">{{ member.phone }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div v-if="balances.length === 0" class="rounded-xl border border-dashed border-sidebar-border/50 py-6 text-center">
                        <CheckCircle2 class="mx-auto mb-1.5 h-6 w-6 text-emerald-400/60" />
                        <p class="text-xs text-muted-foreground">All settled up!</p>
                    </div>
                    ...
                    <div v-else class="space-y-2">
                        ...
                        >
                            <div class="flex items-center gap-2 text-xs">
                                <span class="font-semibold text-red-400">{{ balance.fromName }}</span>
                                <ArrowRight class="h-3 w-3 shrink-0 text-muted-foreground/50" />
                                <span class="font-semibold text-emerald-400">{{ balance.toName }}</span>
                            </div>
                            <p class="mt-1 text-sm font-bold tabular-nums">{{ fmt(balance.amount) }}</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <!-- Add Expense Modal -->
    <Teleport to="body">
        <div v-if="showExpenseModal" class="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/60 p-4 backdrop-blur-sm" @click.self="showExpenseModal = false">
            <div class="my-8 w-full max-w-md overflow-hidden rounded-2xl border border-sidebar-border bg-card shadow-2xl">
                <div class="flex items-center justify-between border-b border-sidebar-border/50 px-5 py-4">
                    ...
                    <button @click="showExpenseModal = false" class="text-muted-foreground hover:text-foreground">
                        <X class="h-4 w-4" />
                    </button>
                </div>
                ...
                <form @submit.prevent="submitExpense" class="space-y-4 p-5">
                    ...
                    <!-- Paid by -->
                    <div>
                        <label class="mb-1.5 block text-xs font-medium text-muted-foreground">Paid by *</label>
                        <select
                        ...
                        <p v-if="form.errors.paid_by_member_id" class="mt-1 text-xs text-red-400">{{ form.errors.paid_by_member_id }}</p>
                    </div>
                    ...
                    <div>
                        <div class="mb-2 flex items-center justify-between">
                            <label class="text-xs font-medium text-muted-foreground">Split amounts *</label>
                            <button
                                type="button"
                                @click="splitEvenly"
                        ...
                        <div class="space-y-2">
                            <div v-for="(split, index) in form.splits" :key="index" class="flex items-center gap-2">
                                <span class="min-w-0 flex-1 truncate text-xs font-medium">{{ memberName(split.group_member_id) }}</span>
                                <input
                        ...
                        <p v-if="form.errors['splits']" class="mt-1 text-xs text-red-400">{{ form.errors['splits'] }}</p>
                    </div>

                    <!-- Notes -->
                    <div>
                        <label class="mb-1.5 block text-xs font-medium text-muted-foreground">Notes (optional)</label>
                        <input
                            v-model="form.notes"
                            type="text"
