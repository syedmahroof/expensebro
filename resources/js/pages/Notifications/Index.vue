...
function typeBg(type: string) {
    if (type === 'warning') { return 'bg-amber-400/15 border-amber-500/20'; }
    if (type === 'success') { return 'bg-emerald-400/15 border-emerald-500/20'; }
    return 'bg-blue-400/15 border-blue-500/20';
}

function typeBadge(type: string) {
    if (type === 'warning') { return 'bg-amber-500/15 text-amber-400'; }
    if (type === 'success') { return 'bg-emerald-500/15 text-emerald-400'; }
    ...
    <div class="flex h-full flex-1 flex-col gap-5 p-4 md:p-6">
        ...
        <div class="flex items-start justify-between gap-3">
            <div>
                ...
                <p class="text-sm text-muted-foreground">
                    {{ unreadCount > 0 ? `${unreadCount} unread` : 'All caught up' }} · {{ notifications.total }} total
                </p>
            </div>
            <button
                v-if="hasUnread"
            ...
            >
                <CheckCheck class="h-3.5 w-3.5 text-emerald-400" /> Mark all read
            </button>
        </div>
        ...
        <div class="grid gap-3 sm:grid-cols-3">
            <div class="rounded-xl border border-blue-500/20 bg-blue-500/5 p-4">
                ...
                <p class="mt-1.5 text-xl font-bold text-blue-400">{{ unreadCount }}</p>
                <p class="mt-0.5 text-xs text-muted-foreground">notifications</p>
            </div>
            <div class="rounded-xl border border-sidebar-border/50 bg-card p-4">
                <p class="text-xs text-muted-foreground">Total</p>
                <p class="mt-1.5 text-xl font-bold">{{ notifications.total }}</p>
                <p class="mt-0.5 text-xs text-muted-foreground">all time</p>
            </div>
            <div class="rounded-xl border border-sidebar-border/50 bg-card p-4">
                <p class="text-xs text-muted-foreground">This Page</p>
                <p class="mt-1.5 text-xl font-bold">{{ notifications.data.length }}</p>
                <p class="mt-0.5 text-xs text-muted-foreground">of {{ notifications.per_page }} per page</p>
            </div>
        </div>

        <!-- Empty state -->
        <div v-if="notifications.data.length === 0" class="flex flex-col items-center justify-center rounded-xl border border-dashed border-sidebar-border/50 py-16 text-center">
            <Bell class="mb-3 h-10 w-10 text-muted-foreground/30" />
            <p class="font-medium text-muted-foreground">No notifications yet</p>
            <p class="mt-1 text-sm text-muted-foreground/60">You're all caught up!</p>
        </div>
        ...
        <div v-else class="space-y-2">
            <div
                ...
                :class="n.read ? 'border-sidebar-border/40 opacity-80' : 'border-sidebar-border/70'"
            >
                <!-- Icon -->
                <div
                    ...
                    :class="typeBg(n.type)"
                >
                    <component :is="typeIcon(n.type)" class="h-4.5 w-4.5" :class="typeColor(n.type)" />
                </div>

                <!-- Content -->
                <div class="min-w-0 flex-1">
                    <div class="flex flex-wrap items-start gap-2">
                        <p class="flex-1 text-sm font-semibold" :class="n.read ? 'text-foreground/70' : 'text-foreground'">
                            {{ n.title }}
                        </p>
                        <div class="flex items-center gap-2 shrink-0">
                            ...
                            >{{ n.type }}</span>
                            <span v-if="!n.read" class="h-2 w-2 rounded-full bg-primary" />
                        </div>
                    </div>
                    <p class="mt-1 text-sm leading-relaxed text-muted-foreground">{{ n.body }}</p>
                    <p class="mt-2 text-[11px] text-muted-foreground/50">{{ timeAgo(n.created_at) }}</p>
                </div>
            </div>
        </div>

        <!-- Pagination -->
        <div v-if="notifications.last_page > 1" class="flex items-center justify-center gap-1.5 pt-2">
            <button
