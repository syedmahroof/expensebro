...
const page = usePage<{ auth: { user: { default_currency: string; locale: string } } }>();
...
<template>
    ...
    <div class="flex flex-col space-y-6">
        ...
        <form @submit.prevent="submit" class="space-y-6 max-w-md">
            ...
            <!-- Default Currency -->
            <div class="rounded-xl border border-sidebar-border/50 bg-card p-5 space-y-4">
                <div class="flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-muted-foreground">All amounts are converted to this for reporting</p>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <p v-if="form.errors.default_currency" class="mt-1 text-xs text-red-400">{{ form.errors.default_currency }}</p>
                </div>

                <div class="rounded-lg border border-amber-500/20 bg-amber-500/5 p-3 text-xs text-amber-400">
                    When you add a transaction in a different currency, the amount is automatically converted to <strong>{{ form.default_currency }}</strong> using current exchange rates.
                </div>
            </div>
            ...
            <div class="rounded-xl border border-sidebar-border/50 bg-card p-5 space-y-4">
                <div class="flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-muted-foreground">UI labels and messages language</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-2">
                    <button
                        ...
                        :class="form.locale === code
                            ? 'border-primary/60 bg-primary/10 text-primary'
                            : 'border-sidebar-border/50 hover:bg-accent'"
                    >
                        <span class="flex-1 truncate text-xs">{{ name }}</span>
                        <Check v-if="form.locale === code" class="h-3.5 w-3.5 shrink-0 text-primary" />
                    </button>
                </div>
                <p v-if="form.errors.locale" class="text-xs text-red-400">{{ form.errors.locale }}</p>
            </div>
