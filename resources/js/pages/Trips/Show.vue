...
import {
    index as tripsIndex,
    update as tripUpdate,
    destroy as tripDestroy,
} from '@/routes/trips';
...
const maxDailySpend = computed(() =>
    Math.max(...props.dailySpend.map((d) => d.amount), 1),
);

const budgetPercent = computed(() => {
    if (!props.stats.budget) {
        return null;
    }
    return Math.min(
        100,
        Math.round((props.stats.totalSpent / props.stats.budget) * 100),
    );
});
...
async function deleteTrip() {
    const ok = await confirm(
        'All expense tags will be unlinked. This cannot be undone.',
        {
    ...
    <div class="flex h-full flex-1 flex-col gap-6 p-4 md:p-6">
        ...
        >
            ...
            <div class="flex shrink-0 items-center gap-2">
                ...
                >
                    <Check class="h-3.5 w-3.5 text-emerald-400" /> Mark
                    Completed
                </button>
                <button
                    @click="deleteTrip"
        ...
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <div
                v-if="stats.budget"
                class="rounded-xl border border-sidebar-border/50 bg-card p-4"
            >
            ...
            <div
                v-if="stats.dailySafeSpend > 0"
                class="rounded-xl border border-sidebar-border/50 bg-card p-4"
            >
                <p class="text-xs text-muted-foreground">Daily Safe Spend</p>
                <p class="mt-1.5 text-xl font-bold text-primary">
                    {{ fmt(stats.dailySafeSpend) }}
                </p>
            </div>
            <div class="rounded-xl border border-sidebar-border/50 bg-card p-4">
        ...
        <!-- Budget progress -->
        <div
            v-if="stats.budget && budgetPercent !== null"
            class="rounded-xl border border-sidebar-border/50 bg-card p-5"
        >
        ...
        <div class="grid gap-6 lg:grid-cols-2">
            ...
            <div class="rounded-xl border border-sidebar-border/50 bg-card p-5">
                ...
                <div
                    v-else
                    class="flex items-end gap-1.5 overflow-x-auto"
                    style="height: 130px"
                >
                    <div
                    ...
                    >
                        ...
                        >
                            <div
                                ...
                                :style="{
                                    height: `${Math.max(4, (day.amount / maxDailySpend) * 100)}%`,
                                }"
        ...
        <!-- Location Hotspots -->
        <div
            v-if="locationHotspots.length > 0"
            class="rounded-xl border border-sidebar-border/50 bg-card p-5"
        >
        ...
        <div>
            ...
            <div
                ...
                class="rounded-xl border border-dashed border-sidebar-border/50 p-8 text-center text-sm text-muted-foreground"
            >
                No transactions tagged to this trip yet.
            </div>
            <div
                v-else
                class="overflow-hidden rounded-xl border border-sidebar-border/50"
            >
                <div class="divide-y divide-sidebar-border/30">
                    <div
                        v-for="tx in transactions"
                        :key="tx.id"
                        class="flex items-center gap-3 px-4 py-3 transition-colors hover:bg-accent/30"
                    >
                        <div
