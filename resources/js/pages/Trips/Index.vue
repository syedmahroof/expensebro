...
function budgetPercent(trip: Trip): number {
    if (!trip.budget || !trip.total_spent) {
        return 0;
    }
    return Math.min(
        100,
        Math.round((Number(trip.total_spent) / Number(trip.budget)) * 100),
    );
}
...
<template>
    ...
    <div class="flex h-full flex-1 flex-col gap-6 p-4 md:p-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-muted-foreground">
                    Track spending across all your travels
                </p>
            </div>
            <button
        ...
        <div
            v-if="trips.length === 0"
            class="flex flex-col items-center justify-center rounded-2xl border border-dashed border-sidebar-border/50 py-20 text-center"
        >
            <div
                class="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-primary/10"
            >
            ...
            >
                ...
                <div v-if="trip.budget">
                    ...
                    >
                        ...
                        <span class="font-medium">
                            ...
                            <span class="text-muted-foreground">
                                / {{ fmt(trip.budget, trip.currency) }}</span
                            >
                        </span>
                    </div>
                    <div class="h-1.5 rounded-full bg-sidebar-border/40">
                        <div
                            ...
                            :class="
                                budgetPercent(trip) >= 90
                                    ? 'bg-red-400'
                                    : budgetPercent(trip) >= 70
                                      ? 'bg-amber-400'
                                      : 'bg-primary'
                            "
                ...
                >
                    <span class="flex items-center gap-1">
                        ...
                        {{ trip.transactions_count }} transaction{{
                            trip.transactions_count !== 1 ? 's' : ''
                        }}
                    </span>
                    <span
                        v-if="!trip.budget && trip.total_spent"
