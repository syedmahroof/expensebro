...
import { Banknote, Bitcoin, Building2, CreditCard, Plus, Wallet, X } from 'lucide-vue-next';
import { computed, ref } from 'vue';
import type { Wallet as WalletType } from '@/types';
import { useConfirm } from '@/composables/useConfirm';
import { destroy, index as walletsIndex, store, update } from '@/routes/wallets';
...
async function archiveWallet(id: number) {
    const ok = await confirm('This wallet will be archived and hidden from your active list.', {
...
const walletColors = ['#6366f1', '#10b981', '#f59e0b', '#ef4444', '#3b82f6', '#8b5cf6', '#ec4899', '#06b6d4'];
</script>
    ...
    <div class="flex h-full flex-1 flex-col gap-6 p-4 md:p-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-muted-foreground text-sm">Manage your accounts and balances</p>
            </div>
            <button
                @click="openCreate"
        ...
        <div class="rounded-xl border border-sidebar-border/50 bg-card p-6">
            <p class="text-muted-foreground mb-1 text-sm">Total Balance</p>
            <p class="text-3xl font-semibold tracking-tight">
                {{ formatAmount(wallets.reduce((sum, w) => sum + Number(w.balance), 0)) }}
            </p>
            <p class="text-muted-foreground mt-1 text-sm">Across {{ wallets.length }} wallet{{ wallets.length !== 1 ? 's' : '' }}</p>
        </div>

        <!-- Wallets Grid -->
        <div v-if="wallets.length === 0" class="flex flex-1 items-center justify-center rounded-xl border border-dashed border-sidebar-border/50 p-12 text-center">
            <div>
                <Wallet class="mx-auto mb-3 h-10 w-10 text-muted-foreground opacity-40" />
                <p class="text-muted-foreground mb-2 text-sm">No wallets yet.</p>
                <button @click="openCreate" class="text-xs text-primary hover:text-primary/80">Add your first wallet →</button>
            </div>
        </div>
        ...
        <div v-else class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            >
                <div class="mb-4 flex items-center justify-between">
                    <div class="flex h-11 w-11 items-center justify-center rounded-xl" :style="{ backgroundColor: wallet.color + '25' }">
                        <component :is="walletTypeIcons[wallet.type] ?? Wallet" class="h-5 w-5" :style="{ color: wallet.color }" />
                    </div>
                    <span v-if="wallet.is_default" class="rounded-full bg-primary/20 px-2 py-0.5 text-xs text-primary">Default</span>
                </div>

                <p class="text-base font-semibold">{{ wallet.name }}</p>
                <p class="text-muted-foreground mb-3 text-xs capitalize">{{ wallet.type }}</p>
                <p class="text-xl font-bold tabular-nums">{{ formatAmount(wallet.balance, wallet.currency) }}</p>

                <div class="mt-4 flex gap-2 opacity-0 transition-opacity group-hover:opacity-100">
                    <button @click="openEdit(wallet)" class="text-muted-foreground hover:text-foreground text-xs transition-colors"> Edit </button>
                    <span class="text-muted-foreground text-xs">·</span>
                    <button @click="archiveWallet(wallet.id)" class="text-xs text-red-400/70 transition-colors hover:text-red-400"> Archive </button>
                </div>
            </div>
        </div>
    </div>
        ...
        <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
            <div class="w-full max-w-md rounded-2xl border border-sidebar-border bg-card p-6 shadow-2xl">
                <div class="mb-6 flex items-center justify-between">
                    <h2 class="text-base font-semibold">{{ editingWallet ? 'Edit Wallet' : 'New Wallet' }}</h2>
                ...
                <form @submit.prevent="submit" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <select v-model="form.type" class="h-10 w-full rounded-lg border border-input bg-background px-3 text-sm outline-none focus:ring-1 focus:ring-ring">
                    ...
                    <div v-if="!editingWallet">
                        <label class="mb-1.5 block text-sm font-medium">Opening Balance</label>
                        <input
                    ...
                    <div class="flex gap-3 pt-2">
                        ...
                        >
                            {{ form.processing ? 'Saving...' : (editingWallet ? 'Update' : 'Create') }}
                        </button>
                    </div>
                </form>
            </div>
