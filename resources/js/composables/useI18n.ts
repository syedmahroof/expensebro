...
export function useI18n() {
    ...
    function t(key: string, fallback?: string): string {
        ...
        const val = deepGet(msgs as Record<string, unknown>, key) ?? deepGet(locales.en as Record<string, unknown>, key);
        return val ?? fallback ?? key;
    }

    const isRtl = computed(() => ['ar', 'ur', 'he', 'fa'].includes(locale.value));
